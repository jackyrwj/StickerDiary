...
**2026-08-03 首次双参考全套实测**：`wan2.7-image-pro` 顺序完成 12/12 张，无失败或重试，总模型等待 129.5 秒，单张 9.9–12.8 秒。圆框眼镜、短黑发和深色穿搭使人物具备基本可识别性，12 个动作语义均清楚；但独立请求之间的脸型、年龄感、发型细节、服装和渲染风格漂移明显，因此当前方案通过“语义、延迟、稳定性和资产处理”，尚未通过“整套一致性”的上线标准。下一次付费全套测试前，必须先改变一致性策略，例如使用模型原生角色一致性能力或不增加用户确认页的内部参考串联。
...
| 12 张之间角色漂移 | 整套不可用 | 测试模型原生角色一致性或内部参考串联，不增加强制锚点确认页，并允许单张重做 |
