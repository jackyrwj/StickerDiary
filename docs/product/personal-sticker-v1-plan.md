...
**实施建议**：不要直接用原始照片分别生成 12 张贴纸。先生成或确定一个“角色锚点”，再将原始参考图与角色锚点共同提供给每个反应生成任务。
...
首次锚点生成后，用户回答“像不像”。如果不像，系统引导用户补充参考图或重新创建锚点，而不是继续浪费 12 次生成。

身份提示词只作为辅助控制，不作为相似度保障。服务端可以明确要求保留脸型、五官间距、发际线和关键配饰，并避免夸张放大眼睛或改变脸部轮廓；但是否“像本人”仍以用户确认的角色锚点为准。首张照片相似度不足时，优先要求补充 2–4 张正脸、轻微侧脸和自然表情参考，而不是仅反复改写提示词。

角色锚点未通过用户确认前，不允许启动 12 张批量生成。这样把相似度问题限制在一次低成本测试中。
...
- 客户端秘密扫描无生产凭证。
- 真实照片可以完成至少一套端到端生成。
- 单张角色锚点必须先通过“像本人”确认，未通过时不得启动 12 张批量生成。
...
| 风险 | 影响 | 应对 |
|---|---|---|
| 单张照片导致角色不像 | 用户立即失去信任 | 支持最多四张参考图，先确认角色锚点再生成整包 |
